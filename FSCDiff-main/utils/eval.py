...
from utils.metrics import Emeasure, Smeasure, WeightedFmeasure, _cal_mae, MaxFmeasure
...
def measure_mea(mask_name):
    ...
    def cal_max_fm(self, pred: np.ndarray, gt: np.ndarray):
    ...
    max_fm = cal_max_fm(MFM, pred, gt)
    return sm, changeable_em, wfm, mae, max_fm
...
def eval(mask_path='./Dataset/TestDataset',
    ...
    for dataset in [dataset_name]:
        ...
        results = {
            ...
            "maxFmeasure": np.mean(np.array(max_fms, dtype=np.float64)),
        }
