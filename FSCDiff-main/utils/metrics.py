...
class MaxFmeasure(object):
    ...
    def step(self, pred: np.ndarray, gt: np.ndarray):
        ...
        max_fm = self.cal_max_fm(pred, gt)
        self.max_fms.append(max_fm)

    def cal_max_fm(self, pred: np.ndarray, gt: np.ndarray) -> float:
        best_threshold = _get_adaptive_threshold(pred, max_value=1)
        max_fm = 0

        for threshold in np.linspace(0, 1, 256):
            binary_pred = pred >= threshold
            intersection = np.logical_and(binary_pred, gt).sum()
            precision = intersection / (np.sum(binary_pred) + _EPS)
            recall = intersection / (np.sum(gt) + _EPS)
            fm = (1 + self.beta**2) * (precision * recall) / (self.beta**2 * precision + recall + _EPS)
            if fm > max_fm:
                max_fm = fm
                if max_fm == 1:  # Early stopping if we reach perfect score
                    break

        return max_fm

    def get_results(self) -> dict:
        max_fm = np.max(np.array(self.max_fms, dtype=_TYPE))
        return dict(max_fm=max_fm)
