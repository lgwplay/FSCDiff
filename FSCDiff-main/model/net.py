...
class net(nn.Module):
    def __init__(self, class_num=2, mask_chans=0, **kwargs):
        ...
        self.dp_backbone = pvt_v2_b1(in_chans=3, mask_chans=mask_chans)
        self.decode_head = Decoder(dims=[64, 128, 320, 512], dim=256, class_num=class_num, mask_chans=mask_chans)
        self.SpaFre1 = SpaFre(channels=64)
        self.SpaFre2 = SpaFre(channels=128)
        self.SpaFre3 = SpaFre(channels=320)
        self.SpaFre4 = SpaFre(channels=512)
        self._init_weights()  # load pretrain
        
    def forward(self,x,timesteps, cond_img,dp):
        Features = []
        features = self.backbone(x, timesteps, cond_img)
        rgbc1, rgbc2, rgbc3, rgbc4 = features
        
        dp_features = self.dp_backbone(x, timesteps, dp)
        dpc1, dpc2, dpc3, dpc4 = dp_features
        F1 = self.SpaFre1(rgbc1,dpc1)
        F2 = self.SpaFre2(rgbc2,dpc2)
        F3 = self.SpaFre3(rgbc3,dpc3)
        F4 = self.SpaFre4(rgbc4,dpc4)
    ...
    def _init_weights(self):
        ...
        # 为深度图backbone加载预训练权重
        dp_pretrained_dict = torch.load(self._download_weights('pvt_v2_b1'))
        dp_model_dict = self.dp_backbone.state_dict()
        dp_pretrained_dict = {k: v for k, v in dp_pretrained_dict.items() if k in dp_model_dict}
        dp_model_dict.update(dp_pretrained_dict)
        self.dp_backbone.load_state_dict(dp_model_dict, strict=False)

    @torch.inference_mode()
    def sample_unet(self, x, timesteps, cond_img,dp):
        return self.forward(x, timesteps, cond_img,dp)

    def extract_features(self, cond_img,dp):
        # do nothing
        return cond_img,dp
...
class SpaFre(nn.Module):
    def __init__(self, channels):
        ...
        self.spa_process = Attention_S(channels * 2, num_heads=8, bias=False)
