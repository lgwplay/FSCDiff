...
class CondUnetWrapper(nn.Module):
    ...
    def sample_unet(self, x, times, conditioning_features,dp):
...
class CondGaussianDiffusion(GaussianDiffusion):
    ...
    def forward(self, img, cond_img,dp,seg=None, extra_cond=None, *args, **kwargs):
        ...
        return self.p_losses(img, times, cond_img,dp,seg, extra_cond, *args, **kwargs)

    def p_losses(self, x_start, times,cond_img,dp,seg=None, extra_cond=None, noise=None, *args, **kwargs):
        ...
        model_out = self.model(torch.cat([x, extra_cond], dim=1), log_snr, cond_img,dp)
    ...
    @torch.no_grad()
    def sample(self, cond_img,dp,extra_cond=None, verbose=True):
    ...
    @torch.no_grad()
    def p_sample_loop(self, shape, cond_img,dp,extra_cond, verbose=True):
        self.history = []
        img = torch.randn(shape, device=self.device)
        conditioning_features,dp = self.model.extract_features(cond_img,dp)
        ...
        for i in tqdm(range(self.num_sample_steps), desc='sampling loop time step', total=self.num_sample_steps,
            ...
            img = self.p_sample(img,conditioning_features,dp,extra_cond, times, times_next)
    ...
    @torch.no_grad()
    def p_sample(self, x, cond,dp,extra_cond, time, time_next):
        batch, *_, device = *x.shape, x.device

        model_mean, model_variance = self.p_mean_variance(x=x, cond=cond,dp=dp,extra_cond=extra_cond, time=time,
                                                          time_next=time_next)
    ...
    def p_sample_g(self, x, cond,dp,extra_cond, time, time_next):
        batch, *_, device = *x.shape, x.device
    ...
    def p_mean_variance(self, x, cond,dp,extra_cond, time, time_next):
...
class ResCondGaussianDiffusion(CondGaussianDiffusion):
    ...
    def p_sample_loop(self, shape, cond_img,dp,extra_cond, verbose=True):
        self.history = []
        img = torch.randn(shape, device=self.device)
        conditioning_features = self.model.extract_features(cond_img,dp)
        steps = torch.linspace(1., 0., self.num_sample_steps + 1, device=self.device)
