...
class PolypObjDataset(data.Dataset):
    def __init__(self, image_root, gt_root,dp_root,trainsize, mean=None, std=None, randomPeper=True,
        ...
        self.dps = [dp_root + f for f in os.listdir(dp_root) if f.endswith('.png')]
        ...
        self.gt_transform = transforms.Compose([
            ...
            transforms.ToTensor()])
        self.dp_transform = transforms.Compose([
            transforms.Resize((self.trainsize, self.trainsize)),
            transforms.ToTensor()])
    ...
    def __getitem__(self, index):
        ...
        dp = self.depth_loader(self.dps[index]) 
        image_size = image.size
        # data augumentation
        image, gt, dp= self.aug_transform(image=np.asarray(image), mask=np.asarray(gt),dp=np.asarray(dp)).values()
        image, gt, dp = albumentations.PadIfNeeded(*image_size[::-1], border_mode=0)(image=image, mask=gt,dp=dp).values()
        image, gt, dp = albumentations.RandomCrop(*image_size[::-1])(image=image, mask=gt,dp=dp).values()
    ...
    def filter_files(self):
        ...
        gts = []
        dps = []
        for img_path, gt_path,dp_path in zip(self.images, self.gts,self.dps):
...
def get_loader(image_root, gt_root, dp_root,batchsize, trainsize,
               shuffle=True, num_workers=12, pin_memory=True):
    dataset = PolypObjDataset(image_root,gt_root,dp_root,trainsize)
...
class test_dataset(data.Dataset):
    def __init__(self, image_root, gt_root,dp_root, testsize, mean=None, std=None):
        super(test_dataset, self).__init__()
        self.testsize = testsize
        self.images = [image_root + f for f in os.listdir(image_root) if f.endswith('.jpg') or f.endswith('.png')]
        self.gts = [gt_root + f for f in os.listdir(gt_root) if f.endswith('.tif') or f.endswith('.png')]
        self.dps = [dp_root + f for f in os.listdir(dp_root) if f.endswith('.tif') or f.endswith('.png')]
        self.images = sorted(self.images)
        self.gts = sorted(self.gts)
        self.dps = sorted(self.dps)
    ...
    def load_data(self):
        ...
        dp = self.dp_loader(self.dps[self.index])
        dp = self.transform(dp).unsqueeze(0)
        ...
        return image, gt,dp,name, image_for_post
    ...
    def __getitem__(self, item):
        ...
        dp = self.depth_loader(self.dps[item])
        dp = self.transform(dp).unsqueeze(0)
        ...
        return {'image': image, 'gt': gt,'dp': dp, 'name': name, 'image_for_post': image_for_post}
